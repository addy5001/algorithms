...
public class GridIllumination {
    ...
    public int[] gridIllumination(int N, int[][] lamps, int[][] queries) {
        ...
        Map<Map.Entry<Integer,Integer>, Boolean> m5 = new HashMap();       // whether lamp is  ON|OFF
        ...
        for(int i=0; i<queries.length; i++){
            ...
            for(int d=0; d<dirs.length; d++){
                ...
                Map.Entry<Integer, Integer> key = Map.entry(x1, y1);
                if(m5.containsKey(key) && m5.get(key)){
                    // the lamp is on, turn it off, so decrement the count of the lamps
                    m1.put(x1, m1.get(x1) - 1);
                    m2.put(y1, m2.get(y1) - 1);
                    m3.put(x1 - y1, m3.get(x1 - y1) - 1);
                    m4.put(x1 + y1, m4.get(x1 + y1) - 1);
                    m5.put(key, false);
                }
            }
        }
